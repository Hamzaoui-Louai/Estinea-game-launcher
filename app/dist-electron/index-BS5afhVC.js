import "os";
import "path";
import { C as CredentialsProviderError, s as setCredentialFeature, j as chain, h as getProfileName } from "./main-kc0GCZR0.js";
import "crypto";
import { p as parseKnownFiles } from "./parseKnownFiles-CFPiUQai.js";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-CiwWhkUR.js");
      const { fromContainerMetadata } = await import("./index-DJepxGxk.js");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-Cp0ExYP8.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-BT4Bb9ij.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-Vm-dKrAT.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-CFeGNn5V.js").then(({ fromTokenFile }) => fromTokenFile({
