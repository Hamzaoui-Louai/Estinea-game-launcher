import "os";
import "path";
import { C as CredentialsProviderError, s as setCredentialFeature, j as chain, h as getProfileName } from "./main-6LQ_pQZv.js";
import "crypto";
import { p as parseKnownFiles } from "./parseKnownFiles-DNpb56UL.js";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-CovNkO_7.js");
      const { fromContainerMetadata } = await import("./index-COzVBI2N.js");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-CFaglCv5.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-DRHKY2VQ.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-CCG1NzLy.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-DvoMZZvB.js").then(({ fromTokenFile }) => fromTokenFile({
