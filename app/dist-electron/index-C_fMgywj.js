import "os";
import "path";
import { C as CredentialsProviderError, s as setCredentialFeature, j as chain, h as getProfileName } from "./main-CngRFuOL.js";
import "crypto";
import { p as parseKnownFiles } from "./parseKnownFiles-BhQtmTLj.js";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-6gYeJP57.js");
      const { fromContainerMetadata } = await import("./index-DPCzARRR.js");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-amka-4UQ.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-C-wEt4pc.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-DnxL4XUB.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-Cdtd6pKL.js").then(({ fromTokenFile }) => fromTokenFile({
