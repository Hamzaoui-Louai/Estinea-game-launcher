import "os";
import "path";
import { C as CredentialsProviderError, s as setCredentialFeature, j as chain, h as getProfileName } from "./main-A8bEEj67.js";
import "crypto";
import { p as parseKnownFiles } from "./parseKnownFiles-CcyZPOlT.js";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-p3YD_E9K.js");
      const { fromContainerMetadata } = await import("./index-uvYrr_7-.js");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-DeqxruPB.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-1kMZ7-KM.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-BFNezHQn.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-PWK3egLX.js").then(({ fromTokenFile }) => fromTokenFile({
