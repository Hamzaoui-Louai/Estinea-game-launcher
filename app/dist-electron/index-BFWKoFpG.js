import "os";
import "path";
import { C as CredentialsProviderError, s as setCredentialFeature, j as chain, h as getProfileName } from "./main-CVza3XXe.js";
import "crypto";
import { p as parseKnownFiles } from "./parseKnownFiles-CLmZW4ti.js";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-BMknAxrl.js");
      const { fromContainerMetadata } = await import("./index-DcunRGdi.js");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-DHQsGuhJ.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-CMmsPJgi.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-DGH-Ts-s.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-Bgc46G2z.js").then(({ fromTokenFile }) => fromTokenFile({
